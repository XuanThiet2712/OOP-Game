...
int PhepThuat::SatThuong(int t) {
	...
	if (loaiPhep == "Hoa" || loaiPhep == "Loi") {
		...
		if (n == 0) {
			cout << RED << ">> Khong du mana de tan cong!\n" << RESET;
			return 0;
		}
		
		// Damage cộng dồn
		float D = base * (n + 0.01f * n * (n - 1) / 2.0f);
		tongDamage = (int)D;
		
		// Mana còn lại
		int manaConLai = tongMana - n * manaTieuThu;
		nguoiDungPhep->setMana(min(manaConLai, manaMax));
	}
	...
	if (loaiPhep == "Loi") {
		...
		float nhanCuoi = min(1.0f + max(0, n1 - 1) * 0.01f, 3.0f);
