...
//	}
	...
	int SatThuong(int t) override {
		...
		cout <<string(22,'=')<<endl;
		cout <<">> Bang dan da duoc nap day lai nhu cu"<<endl;
...
public:
	...
	Kiem(string ten, int st, float td, int db) : VuKhi(ten, st, td), doBen(db){}
	~Kiem() {}
	...
	int SatThuong(int t) override {
		...
		int tongDamage = getSatThuongCoBan()/(float)(2*doBen) * n * (2*doBen - n + 1 ) ;
